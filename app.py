...
def custom_postprocess_text(text: str) -> str:
    """
    Выполняем доработку результата рерайта:
    
    1. Удаляем строки или фрагменты, начинающиеся с "Note:" (чтобы не попадали пояснения на китайском).
    2. Удаляем в начале ответа нежелательные слова (например, "fucking", "explicit", "intense").
    3. Возвращаем слово "fuck" вместо цензурированного "F***".
    4. Удаляем китайские символы.
    5. Удаляем эмодзи.
    6. Убираем все двойные кавычки и лишние пробелы.
    """
    ...
    # 2. Удаляем в начале строки нежелательные слова (например, "fucking", "explicit", "intense")
    # Если в начале ответа есть повторяющиеся слова с разделителями (запятые, двоеточия, тире), то они будут удалены
    text = re.sub(r'^(?:(?:fucking|explicit|intense)[\s,:\-]+)+', '', text, flags=re.IGNORECASE)
    
    # 3. Возвращаем оригинальное слово "fuck", заменяя варианты вроде "F***"
    text = re.sub(r'\bF\*+\b', 'fuck', text, flags=re.IGNORECASE)
    
    # 4. Удаляем все китайские символы (диапазон Unicode для CJK)
    text = re.sub(r'[\u4e00-\u9fff]+', '', text)
    
    # 5. Удаляем эмодзи
    emoji_pattern = re.compile("["
                               u"\U0001F600-\U0001F64F"  # эмодзи - лица и эмоции
...
def process_single_row(
    ...
    # Постобработка: убираем нежелательные фрагменты, заменяем цензурированные слова, удаляем эмодзи и китайские символы
