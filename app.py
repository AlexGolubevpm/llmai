...
# Создаем 5 вкладок: Обработка текста, Перевод текста, Разделение файла, Постобработка, Выбор столбцов
...
########################################
# Вкладка 5: Выбор столбцов из CSV с добавлением столбца без кавычек
########################################
with tabs[4]:
        ...
        try:
            ...
            if not available_columns:
                ...
                if selected_columns:
                    ...
                    # Добавляем дополнительный столбец (6-й столбец) без кавычек
                    no_quotes_source = st.selectbox("Выберите столбец, из которого сформировать столбец без кавычек", options=selected_columns, key="no_quotes_column")
                    filtered_df["без кавычек"] = filtered_df[no_quotes_source].astype(str).apply(lambda x: x.replace('"', ''))
                    
                    st.write("### 📋 Предпросмотр итогового файла (с дополнительным столбцом)")
                    st.dataframe(filtered_df.head(10))
